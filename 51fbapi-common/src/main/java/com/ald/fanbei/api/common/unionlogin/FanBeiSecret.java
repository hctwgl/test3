...
public class FanBeiSecret {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("encoded phone = " + phoneBase64EncodeStr);

        //解密
        byte[] phoneDecodeBase64Byte =   Base64.decodeBase64(phoneBase64EncodeStr);
        byte[] phoneDecodeAesByte = decode(secret_key,phoneDecodeBase64Byte );

        String finalStr = new String(phoneDecodeAesByte, "utf-8");
        System.out.println("decodedStr = " + finalStr.trim());//注意 去掉后空格


        //签名
        TreeMap<String, String> paramMap =new TreeMap<>();
        paramMap.put("phone",phoneBase64EncodeStr);
        paramMap.put("channel",channel);
        //非空参数进行签名计算
        String sign= signParams(secret_key,paramMap);

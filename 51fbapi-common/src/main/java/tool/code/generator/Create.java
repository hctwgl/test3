...
public class Create {
	...
	public void test() {
		...
		String table = "af_boluome_activity_user_rebate";
		...
		try {
			...
			MybatisGenerate.generateCode(db, url, MysqlUser, mysqlPassword, database, table, commonName, packageName,
					batisName, moduleName, classAuthor, functionName, classNamePrefix);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private static String getClassName(String tableName) {
		...
		while (beanName.indexOf("_") >= 0) {
			...
			beanName = beanName.substring(0, firstSpe) + beanName.substring(firstSpe + 1, firstSpe + 2).toUpperCase()
					+ beanName.substring(firstSpe + 2);
		}
		return beanName;
	}

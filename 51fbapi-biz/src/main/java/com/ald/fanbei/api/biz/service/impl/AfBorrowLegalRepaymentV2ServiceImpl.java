...
public class AfBorrowLegalRepaymentV2ServiceImpl extends UpsPayKuaijieServiceAbstract implements AfBorrowLegalRepaymentV2Service {
    ...
    /**
     * 新版还钱函 参考
     * {@link com.ald.fanbei.api.biz.service.impl.AfRepaymentBorrowCashServiceImpl}
     * .createRepayment()
     */
    @Override
	...
	if (bo.cardId > 0) {// 银行卡支付
	    ...
	    KuaijieRepayV2Bo bizObject = new KuaijieRepayV2Bo(repayment, bo);
...
//	afRepaymentBorrowCashDao.status2ProcessKuaijie(payTradeNo, kuaijieRepaymentBo.getRepayment().getRid());
	...
	// 因为有用户会多还几分钱，所以加个安全金额限制，当还款金额 > 用户应还金额+1元 时，返回错误
	 TODO 去除多还钱的限制 By ZJF，允许用户一次还两笔借款， if (offlineRepayAmountYuan.compareTo(restAmount.add(BigDecimal.valueOf(200))) > 0) {

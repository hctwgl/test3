 ...
 *
 */
@Component("insuranceAuthCallbackExecutor")
public class InsuranceAuthCallbackExecutor implements Executor {
	...
	logger.info("start insurance auth callback execute");
	...
	if (StringUtils.equals(authCallbackBo.getResult(), RiskAuthStatus.SUCCESS.getCode())) {
		...
		if (StringUtils.equals("Y", basicStatus)) {
		    RiskQuotaRespBo respBo = riskUtil.userSupplementQuota(ObjectUtils.toString(userId), new String[] { RiskScene.INSURANCE_XJD_PASS.getCode() }, RiskSceneType.XJD.getCode());
		    // 提额成功
			...
			if (StringUtils.equals(RiskRaiseResult.PASS.getCode(), raiseStatus)) {
			    ...
			    AfAuthRaiseStatusDo raiseStatusDo = afAuthRaiseStatusService.buildAuthRaiseStatusDo(userId, AuthType.INSURANCE.getCode(), LoanType.CASH.getCode(), "Y", new BigDecimal(amount), new Date());
			    // 提额成功，记录提额状态
			    afAuthRaiseStatusService.saveOrUpdateRaiseStatus(raiseStatusDo);
			} else {
			    AfAuthRaiseStatusDo raiseStatusDo = afAuthRaiseStatusService.buildAuthRaiseStatusDo(userId, AuthType.INSURANCE.getCode(), LoanType.CASH.getCode(), "F", BigDecimal.ZERO, new Date());
			    // 提额成功，记录提额状态
			    afAuthRaiseStatusService.saveOrUpdateRaiseStatus(raiseStatusDo);
			}
		    ...
		    RiskQuotaRespBo respBo = riskUtil.userSupplementQuota(ObjectUtils.toString(userId), new String[] { RiskScene.INSURANCE_XJD_UNPASS.getCode() }, RiskSceneType.XJD.getCode());
		    // 提额成功
		    if (respBo != null && respBo.isSuccess()) {
			String raiseStatus = respBo.getData().getResults()[0].getResult();
			if (StringUtils.equals(RiskRaiseResult.PASS.getCode(), raiseStatus)) {
		...
		try {
		    afAuthRaiseStatusDo = afAuthRaiseStatusService.getByPrdTypeAndAuthType(SceneType.BLD_LOAN.getName(), AuthType.INSURANCE.getCode(), userId);
		    if (afUserAuthService.getAuthRaiseStatus(afAuthRaiseStatusDo, SceneType.CASH.getName(), AuthType.INSURANCE.getCode(), afUserAuthDo.getGmtJinpo())) {
			RiskQuotaRespBo respBo = riskUtil.userSupplementQuota(ObjectUtils.toString(userId), new String[] { RiskScene.INSURANCE_BLD.getCode() }, RiskSceneType.BLD.getCode());
			// 提额成功
			if (respBo != null && respBo.isSuccess()) {
				...
				afUserAccountSenceService.saveOrUpdateAccountSence(totalAccountSenceDo);
				AfAuthRaiseStatusDo raiseStatusDo = afAuthRaiseStatusService.buildAuthRaiseStatusDo(userId, AuthType.INSURANCE.getCode(), LoanType.BLD_LOAN.getCode(), "Y", new BigDecimal(bldAmount), new Date());
				// 提额成功，记录提额状态
				afAuthRaiseStatusService.saveOrUpdateRaiseStatus(raiseStatusDo);
			    } else {
				AfAuthRaiseStatusDo raiseStatusDo = afAuthRaiseStatusService.buildAuthRaiseStatusDo(userId, AuthType.INSURANCE.getCode(), LoanType.BLD_LOAN.getCode(), "F", BigDecimal.ZERO, new Date());
				// 提额成功，记录提额状态
				afAuthRaiseStatusService.saveOrUpdateRaiseStatus(raiseStatusDo);
			    }
			}
	    ...
	    afAuthRaiseStatusDo = afAuthRaiseStatusService.getByPrdTypeAndAuthType(SceneType.ONLINE.getName(), AuthType.INSURANCE.getCode(), userId);
	    if (afUserAuthService.getAuthRaiseStatus(afAuthRaiseStatusDo, SceneType.ONLINE.getName(), AuthType.INSURANCE.getCode(), afUserAuthDo.getGmtJinpo())) {
		// 线上分期提额

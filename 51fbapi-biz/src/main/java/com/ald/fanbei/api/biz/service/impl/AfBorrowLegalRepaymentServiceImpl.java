...
public class AfBorrowLegalRepaymentServiceImpl extends ParentServiceImpl<AfBorrowLegalOrderRepaymentDo, Long> implements AfBorrowLegalRepaymentService {
	 ...
	 * @return
	 */
	public Map<String, Object> repay(final BigDecimal repaymentAmount,final BigDecimal actualAmount,final AfUserCouponDto coupon,final BigDecimal rebateAmount,
		...
		final String payTradeNo = generatorClusterNo.getRepaymentBorrowCashNo(now);
		final String name = typeName;

		final AfRepaymentBorrowCashDo repayment = buildRepayment(BigDecimal.ZERO, repaymentAmount, payTradeNo, now, actualAmount, coupon, rebateAmount, borrow, cardId, payTradeNo, name, userId);
		afRepaymentBorrowCashDao.addRepaymentBorrowCash(repayment);// 新增还款记录,移至事务外，保证回调先回用户这笔还款发起有记录产生

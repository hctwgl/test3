...
public class AfBorrowLegalRepaymentServiceImpl extends ParentServiceImpl<AfBorrowLegalOrderRepaymentDo, Long> implements AfBorrowLegalRepaymentService {
    ...
    private void doRepay(RepayBo bo, AfRepaymentBorrowCashDo repayment, AfBorrowLegalOrderRepaymentDo legalOrderRepayment,String bankPayType) {
		if (bo.cardId > 0) {// 银行卡支付
			...
			UpsCollectRespBo respBo = upsUtil.collect(bo.tradeNo, bo.actualAmount, bo.userId.toString(), 
						...
						bank.getCardNumber(), bo.userDo.getIdNumber(), Constants.DEFAULT_PAY_PURPOSE, bo.name, "02", PayOrderSource.REPAY_CASH_LEGAL.getCode(),bankPayType);

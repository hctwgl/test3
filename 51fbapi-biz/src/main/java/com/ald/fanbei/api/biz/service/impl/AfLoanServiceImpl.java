 ...
 * @author ZJF
 * @version 1.0.0 初始化
 * @date 2018-01-19 16:50:31 Copyright 本内容仅限于杭州阿拉丁信息科技股份有限公司内部传阅，禁止外泄以及用于其他的商业目的
 */
...
public class AfLoanServiceImpl extends ParentServiceImpl<AfLoanDo, Long> implements AfLoanService {
	...
	public void doLoan(ApplyLoanBo bo) {
		...
		try {
			...
			final List<Object> objs = afLoanPeriodsService.resolvePeriods(reqParam.amount, userId, reqParam.periods,
					loanNo, bo.reqParam.prdType);
			...
			AfUserAccountDo userAccount = afUserAccountDao.getUserAccountInfoByUserId(userId);
			// 调用UPS打款
			UpsDelegatePayRespBo upsResult = upsUtil.delegatePay(bo.reqParam.amount, userAccount.getRealName(),
					bankCard.getCardNumber(), userId.toString(), bankCard.getMobile(), bankCard.getBankName(),
					bankCard.getBankCode(), Constants.DEFAULT_LOAN_PURPOSE, "02", UserAccountLogType.LOAN.getCode(),
					loanDo.getRid().toString());
			...
			try {
				...
				smsUtil.sendSmsToDhst(userAccount.getUserName(),
						String.format(Documents.LOAN_SUCC_SMS_MSG, lastBankCode));
				jpushService.pushUtil(Documents.LOAN_SUCC_TITLE, String.format(Documents.LOAN_SUCC_MSG, lastBankCode),
	...
	private void saveLoanRecords(final ApplyLoanBo bo, final AfLoanDo loanDo, final List<AfLoanPeriodsDo> periodDos,
			final AfUserBankcardDo bankCard) {
	...
	private void weakRiskCheck(ApplyLoanBo bo, AfLoanDo tarLoanDo) {
		...
		RiskVerifyRespBo verifyBo = riskUtil.verifyNew(userId.toString(), tarLoanDo.getLoanNo(), tarLoanDo.getPrdType(),
				WeakRiskSceneType.valueOf(bo.reqParam.prdType).getCode(), tarLoanDo.getCardNo(), bo.reqParam.appType,
				bo.reqParam.ip, bo.reqParam.blackBox, riskOrderNo, bo.userName, tarLoanDo.getAmount(),
				tarLoanDo.getTotalInterestFee().add(tarLoanDo.getTotalServiceFee()), DateUtil.getNow(), "贷款",
				StringUtils.EMPTY, null, null, 0l, tarLoanDo.getCardName(), null, "", riskDataMap,
				bo.reqParam.bqsBlackBox, null);
	...
	public void dealLoanSucc(Long loanId, String tradeNoOut) {
		...
		if (AfLoanStatus.TRANSFERRED.name().equals(status)) {// 已经处理过，防重复回调
			logger.warn("DealLoanSucc, transfer has succ, repeat UPS invoke! loanId=" + loanId + ",tradeNoOut="
					+ tradeNoOut);
			return;
		}

		if (AfLoanStatus.CLOSED.name().equals(status)) { // 已失败订单，但UPS仍回调成功，日志打点记录
			logger.warn("DealLoanSucc, transfer has fail, but still callback! original status= " + status + ",loanId="
					+ loanId + ",tradeNoOut=" + tradeNoOut);
		}
	...
	private List<LoanHomeInfoBo> dealHomeLogin(AfUserAccountDo userAccount, List<AfLoanProductDo> prdDos) {
		...
		for (AfLoanProductDo prdDo : prdDos) {
			...
			AfUserAccountSenceDo accScene = afUserAccountSenceService.getByUserIdAndScene(prdType,
	...
	private void dealHomeLoginLoan(LoanHomeInfoBo bo, AfLoanDo lastLoanDo) {
		...
		if (AfLoanStatus.FINISHED.name().equals(status) || AfLoanStatus.CLOSED.name().equals(status)) {
			bo.hasLoan = false;
			return;
		}
		AfLoanRepaymentDo processLoanRepayment = afLoanRepaymentService
				.getProcessLoanRepaymentByLoanId(lastLoanDo.getRid());

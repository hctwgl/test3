...
public class AfResourceServiceImpl implements AfResourceService {
	...
	public List<AfResourceDo> getConfigByTypes(String type) {
		...
		Map<String, List<AfResourceDo>> data = (Map<String, List<AfResourceDo>>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_CONFIG_TYPES_LIST.getCode());
		data = (data == null ? new HashMap<String, List<AfResourceDo>>() : data);
		if (data.get(type) == null) {
			list = afResourceDao.getConfigByTypes(type);
			data.put(type, list);
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_CONFIG_TYPES_LIST.getCode(), SerializeUtil.serialize(data));
		} else {
			list = data.get(type);
		}
		return list;
	...
	public List<AfResourceDo> getResourceListByType(String type) {
		List<AfResourceDo> list;
		Map<String, List<AfResourceDo>> data = (Map<String, List<AfResourceDo>>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_TYPE_LIST.getCode());
		data = (data == null ? new HashMap<String, List<AfResourceDo>>() : data);
		if (data.get(type) == null) {
			list = afResourceDao.getResourceListByType(type);
			data.put(type, list);
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_TYPE_LIST.getCode(), SerializeUtil.serialize(data));
		} else {
			list = data.get(type);
		}
		return list;
	...
	public AfResourceDo getSingleResourceBytype(String type) {
		...
		Map<String, AfResourceDo> data = (Map<String, AfResourceDo>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_TYPE_DO.getCode());
		...
		if (data.get(type) == null) {
			...
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_TYPE_DO.getCode(), SerializeUtil.serialize(data));
		} else {
	...
	public AfResourceDo getConfigByTypesAndSecType(String type, String secType) {
		AfResourceDo afResourceDo;
		Map<String, AfResourceDo> data = (Map<String, AfResourceDo>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_TYPE_SEC_DO.getCode());
		data = (data == null ? new HashMap<String, AfResourceDo>() : data);
		if (data.get(type+secType) == null) {
			...
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_TYPE_SEC_DO.getCode(), SerializeUtil.serialize(data));
		} else {
	...
	public List<AfResourceDo> getResourceListByTypeOrderBy(String type) {
		List<AfResourceDo> list;
		Map<String, List<AfResourceDo>> data = (Map<String, List<AfResourceDo>>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_TYPE_LIST_ORDER_BY.getCode());
		data = (data == null ? new HashMap<String, List<AfResourceDo>>() : data);
		if (data.get(type) == null) {
			list = afResourceDao.getResourceListByTypeOrderBy(type);
			data.put(type, list);
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_TYPE_LIST_ORDER_BY.getCode(), SerializeUtil.serialize(data));
		} else {
			list = data.get(type);
		}
		return list;
	...
	public AfResourceDo getResourceByResourceId(Long rid) {
		AfResourceDo afResourceDo;
		Map<String, AfResourceDo> data = (Map<String, AfResourceDo>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_ID_DO.getCode());
		data = (data == null ? new HashMap<String, AfResourceDo>() : data);
		if (data.get(rid+"") == null) {
			afResourceDo = afResourceDao.getResourceByResourceId(rid);
			data.put(rid+"", afResourceDo);
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_ID_DO.getCode(), SerializeUtil.serialize(data));
		} else {
	...
	public List<AfResourceDo> getOneToManyResourceOrderByBytype(String type) {
		List<AfResourceDo> list;
		Map<String, List<AfResourceDo>> data = (Map<String, List<AfResourceDo>>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_ONE_TO_MANY_TYPE_LIST.getCode());
		data = (data == null ? new HashMap<String, List<AfResourceDo>>() : data);
		if (data.get(type) == null) {
			list = afResourceDao.getOneToManyResourceOrderByBytype(type);
			data.put(type, list);
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_ONE_TO_MANY_TYPE_LIST.getCode(), SerializeUtil.serialize(data));
		} else {
			list = data.get(type);
		}
		return list;
	...
	public List<AfResourceDo> getResourceHomeListByTypeOrderBy(String type) {
		List<AfResourceDo> list;
		Map<String, List<AfResourceDo>> data = (Map<String, List<AfResourceDo>>) bizCacheUtil.getObject(CacheConstants.RESOURCE.RESOURCE_HOME_LIST_ORDER_BY.getCode());
		data = (data == null ? new HashMap<String, List<AfResourceDo>>() : data);
		if (data.get(type) == null) {
			list = afResourceDao.getResourceHomeListByTypeOrderBy(type);
			data.put(type, list);
			bizCacheUtil.saveObject(CacheConstants.RESOURCE.RESOURCE_HOME_LIST_ORDER_BY.getCode(), SerializeUtil.serialize(data));
		} else {
			list = data.get(type);
		}
		return list;

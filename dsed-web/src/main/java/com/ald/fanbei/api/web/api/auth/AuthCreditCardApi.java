		...
		if (afResource == null || afResource.getValue().equals(YesNoStatus.YES.getCode())) {
			...
			if (afResource.getValue1().equals(YesNoStatus.YES.getCode())
				...
				return new ApiHandleResponse(requestDataVo.getId(),
						FanbeiExceptionCode.CREDIT_CERTIFIED_UNDER_MAINTENANCE);
			}
		}
		// 只对强风控认证通过的打开
		if (afResource.getValue2().equals(YesNoStatus.YES.getCode())
				&& !afUserAuthDo.getBasicStatus().equals(YesNoStatus.YES.getCode())) {
			return new ApiHandleResponse(requestDataVo.getId(), FanbeiExceptionCode.CREDIT_CERTIFIED_UNDER_MAINTENANCE);
		}

...
//        fileName = "爱上街合同";
    ...
    private void createCompanySeal(DsedUserSealDo afUserSealDo,Map map) {
        if (null == afUserSealDo || null == afUserSealDo.getUserAccountId()){// 账户标识
            ...
            if(null != accountId){
                addSealResult = createSealOrganize(accountId,
                        "STAR", "RED", "", "");
            }else {
            ...
            insertUserSeal(afUserSealDo1);
        }else if (null == afUserSealDo.getUserSeal() || "" == afUserSealDo.getUserSeal()){//公司的印章base64
            AddSealResult addSealResult = createSealOrganize(afUserSealDo.getUserAccountId(),
                    "STAR", "RED", "", "");
            DsedUserSealDo afUserSealDo1 = new DsedUserSealDo();
            ...
            updateUserSealByUserId(afUserSealDo1);
        }
    }
    ...
    public String addOrganize(Map map) {
        ...
        if (150016 == r.getErrCode() || r.getMsg().contains("账户已存在")){
            if ("浙江楚橡信息科技有限公司".equals(name)){
                DsedUserSealDo mhSeal = selectUserSealByUserId(-4l);

...
public class MobileChargeApi implements ApiHandle {
	...
	public ApiHandleResponse process(RequestDataVo requestDataVo, FanbeiContext context, HttpServletRequest request) {
		...
		if(bankId<0){//微信支付
			map = afOrderService.createMobileChargeOrder(null,context.getUserName(),userId, coupon, money, mobile, rebateAmount,bankId,"",afUserAccountDo,blackBox);
			...
			map = afOrderService.createMobileChargeOrder(card,context.getUserName(),userId, coupon, money, mobile, rebateAmount,bankId,request.getRemoteAddr(),afUserAccountDo,blackBox);

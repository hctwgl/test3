	...
	public ApiHandleResponse process(RequestDataVo requestDataVo, FanbeiContext context, HttpServletRequest request) {
		...
		try {
			...
			if (null != nTbkItemList && nTbkItemList.size() > 0) {
				...
				BigDecimal saleAmount = NumberUtil.objToBigDecimalDefault(item.getReservePrice(), BigDecimal.ZERO);

			    BigDecimal rebateAmount = NumberUtil.objToBigDecimalDefault(item.getTkRate(), BigDecimal.ZERO).divide(new BigDecimal(100)).multiply(saleAmount).divide(new BigDecimal(100));

				String nick = item.getNick();
				data.put("goodsName", title);
				data.put("goodsIcon", pictUrl);
				data.put("rebateAmount", rebateAmount);
				data.put("saleAmount", item.getReservePrice());

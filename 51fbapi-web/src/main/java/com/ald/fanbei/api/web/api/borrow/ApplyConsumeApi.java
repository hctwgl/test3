...
public class ApplyConsumeApi implements ApiHandle {
	...
	public ApiHandleResponse process(RequestDataVo requestDataVo, FanbeiContext context, HttpServletRequest request) {
		...
		BigDecimal useableAmount = userDto.getAuAmount().subtract(userDto.getUsedAmount())
				.subtract(userDto.getFreezeAmount());
		...
		long result = afBorrowService.dealConsumeApply(userDto, amount, card.getRid(), goodsId, openId, numId, name,
				nper);
		if (result > 0) {
			// 直接打款
			if (userDto.getCreditScore() >= Integer.valueOf(resourceInfo.getValue())) {
				// 转账处理
				UpsDelegatePayRespBo upsResult = upsUtil.delegatePay(amount, userDto.getRealName(),
						card.getCardNumber(), userId + "", card.getMobile(), card.getBankName(), card.getBankCode(),
						Constants.DEFAULT_BORROW_PURPOSE, "02", UserAccountLogType.CONSUME.getCode(), result + "");

...
public class TestController {
	...
	try {
		...
		// throw new
		// FanbeiException(FanbeiExceptionCode.RISK_RAISE_QUOTA_ERROR);
		// }
    ...
    @RequestMapping("/boluome/Order/url")
    @ResponseBody
    public String getBoluomeOrderUrl() {
	try {
	    ...
	    return BoluomeCore.buildOrderDetailsQueryUrl(params);
	} catch (UnsupportedEncodingException e) {
	    logger.error("getBoluomeOrderUrl error:", e);
	    return e.getMessage();
	}
    }
    ...
    // public String goodsRequest(HttpServletRequest request,
    ...
    //
    // String reportId = TongdunUtil.applyPreloan("362525198601022112", "陈金虎",
    // "15958119936", "410228573@qq.com");
    ...
    // produces = "application/json;charset=utf-8")
    // public String batchRequest(HttpServletRequest request,
    // HttpServletResponse response) throws IOException {
    // request.setCharacterEncoding(Constants.DEFAULT_ENCODE);
    // response.setContentType("application/json;charset=utf-8");
    ...
    // produces = "application/json;charset=utf-8")
    // public String addressListRequest(HttpServletRequest request,
    // HttpServletResponse response) throws IOException {
    // request.setCharacterEncoding(Constants.DEFAULT_ENCODE);
    // response.setContentType("application/json;charset=utf-8");
    ...
    // List<AfUserAccountDto> list =
    // afUserAccountService.getUserAndAccountListWithHasRealName(query);
    // for (int i = 0; i < list.size(); i++) {
    // List<AfAuthContactsDo> contacts =
    // afAuthContactsService.getContactsByUserId(list.get(i).getUserId());
    // riskUtil.addressListPrimaries(list.get(i).getUserId().toString(),
    // contacts);
    // }
    // }
    // return "succ";
    ...
    // * @return
    // */
    // @RequestMapping(value = { "/SyncAddressList/toRiskManagement" }, method =
    // RequestMethod.GET, produces = "application/json;charset=utf-8")
    // @ResponseBody
    // public String SyncAddressListRequest(HttpServletRequest request,
    // HttpServletResponse response) throws IOException {
    // request.setCharacterEncoding(Constants.DEFAULT_ENCODE);
    // response.setContentType("application/json;charset=utf-8");
    ...
    // query.setPageNo(j);
    // query.setPageSize(10);
    // List<AfUserAuthDo> list =
    // afUserAuthService.getUserAuthListWithIvs_statusIsY(query);
    // for (int i = 0; i < list.size(); i++) {
    // AfContactsOldDo afContactsOldDo =
    // afContactsOldService.getAfContactsByUserId(list.get(i).getUserId());
    // if (null != afContactsOldDo) {
    // String moblieBook = afContactsOldDo.getMobileBook();
    // String formatMoblieBook =
    // moblieBook.substring(moblieBook.indexOf("\"")+1,moblieBook.lastIndexOf("\""));
    //
    ...
    // * @return
    // */
    // @RequestMapping(value = { "/SyncOneAddress" }, method =
    // RequestMethod.GET, produces = "application/json;charset=utf-8")
    // @ResponseBody
    // public String SyncAddressRequest(HttpServletRequest request,
    // HttpServletResponse response) throws IOException {
    // request.setCharacterEncoding(Constants.DEFAULT_ENCODE);
    // response.setContentType("application/json;charset=utf-8");
    //
    ...
    // AfContactsOldDo afContactsOldDo =
    // afContactsOldService.getAfContactsByUserId(userId);
    // String moblieBook = afContactsOldDo.getMobileBook();
    // String formatMoblieBook =
    // moblieBook.substring(moblieBook.indexOf("\"")+1,moblieBook.lastIndexOf("\""));
    //
    ...
    // contacts.add(afAuthContactsDo);
    // }
    // RiskAddressListRespBo riskAddressListRespBo =
    // riskUtil.addressListPrimaries(userId.toString(), contacts);
    ...
    // @RequestMapping(value = { "/dealWithBoluomeBorrow" }, method =
    // RequestMethod.POST, produces = "application/json;charset=utf-8")
    // @ResponseBody
    // public String dealWithBoluomeBorrow(@RequestBody String body,
    ...
    // afUserAccountService.getUserAccountByUserId(orderInfo.getUserId());
    // afBorrowService.dealAgentPayConsumeRisk(userAccountInfo,
	...
	// NumberUtil.objToBigDecimalDefault(json.getString("refundAmount"),
	// null);
	String payTradeNo = json.getString("payTradeNo");
	...
	if (null != afRepaymentBorrowCashDo) {
	    ...
	    // BigDecimal refundAmount = new BigDecimal(0.01).setScale(2,
	    ...
	    UpsDelegatePayRespBo upsResult = upsUtil.delegatePay(refundAmount, userDo.getRealName(), card.getCardNumber(), card.getUserId() + "", card.getMobile(), card.getBankName(), card.getBankCode(), Constants.DEFAULT_REFUND_PURPOSE, "02", OrderType.MOBILE.getCode(), "");
	    String refundNo = generatorClusterNo.getRefundNo(new Date());
	    if (!upsResult.isSuccess()) {
		AfOrderRefundDo afOrderRefundDo = BuildInfoUtil.buildOrderRefundDo(refundNo, refundAmount, refundAmount, afRepaymentBorrowCashDo.getUserId(), 0l, "", OrderRefundStatus.FAIL, PayType.BANK, card.getCardNumber(), card.getBankName(), "用户现金借中重复还款后的退款", RefundSource.PLANT_FORM.getCode(), upsResult.getOrderNo());
		afOrderRefundDao.addOrderRefund(afOrderRefundDo);
		message = "Fail!";
	    } else {
		AfOrderRefundDo afOrderRefundDo = BuildInfoUtil.buildOrderRefundDo(refundNo, refundAmount, refundAmount, afRepaymentBorrowCashDo.getUserId(), 0l, "", OrderRefundStatus.FINISH, PayType.BANK, card.getCardNumber(), card.getBankName(), "用户现金借中重复还款后的退款", RefundSource.PLANT_FORM.getCode(), upsResult.getOrderNo());
	...
	try {
	    ...
	    List<String> orderNumIdsList = CollectionConverterUtil.convertToListFromList(list, new Converter<AfOrderDo, String>() {
		@Override
	...
	// String orderNo = riskUtil.getOrderNo("over",
	// identity.substring(identity.length() - 4, identity.length()));
	// List<RiskOverdueBorrowBo> boList = new
	// ArrayList<RiskOverdueBorrowBo>();
	// RiskOverdueBorrowBo bo = new RiskOverdueBorrowBo();
	// bo.setBorrowNo("jk2017071020281800843");
	// bo.setOverdueDays(0);
	// bo.setOverdueTimes(1);
	// boList.add(bo);
	// logger.info("dealWithSynchronizeOverduedOrder begin orderNo = {} , boList = {}",
	// orderNo, boList);
	...
	for (AfOrderDo orderInfo : list) {
	    ...
	    AfBorrowDo borrow = buildAgentPayBorrow(name, BorrowType.TOCONSUME, orderInfo.getUserId(), orderInfo.getActualAmount(), orderInfo.getNper(), BorrowStatus.APPLY.getCode(), orderInfo.getRid(), orderInfo.getOrderNo(), orderInfo.getBorrowRate(), orderInfo.getInterestFreeJson());
		...
		AfUserVirtualAccountDo virtualAccountInfo = BuildInfoUtil.buildUserVirtualAccountDo(orderInfo.getUserId(), orderInfo.getActualAmount(), orderInfo.getActualAmount(), orderInfo.getRid(), orderInfo.getOrderNo(), virtualCode);
		// 增加虚拟商品记录
	...
	// String appid =
	// AesUtil.decrypt("Ehw14/ML0cbFSiBoVFC1mu6iw/dWDLmwlSlJTCWC/veLZVdz4LtvQ7My3Rfnuzwq/du56FGZDN1TRs9yv/Zn+4N2RXBD1dBHtugwJDhi3Bs=","Cw5bM6x@6sH$2dlw^3JueH");
	// String puk =
	// AesUtil.decrypt("QRaDVWb2pC9by28Rxt8sMQ==","Cw5bM6x@6sH$2dlw^3JueH");
	// String pik =
	// AesUtil.decrypt("fnzwqgFMW/RyuzDKRkH9uP/XN6RgBH5QkvtGwHR2gVs=","Cw5bM6x@6sH$2dlw^3JueH");
	//
	// AesUtil.encryptToBase64(appid,"Cw5bM6x@6sH$2dlw^3JueH");

	// Map m = yiBaoUtility.createOrder(BigDecimal.TEN,"hk"+String.valueOf
	// (new Date().getTime()/1000),20158l,PayOrderSource.BORROWCASH);
	// String token = m.get("token").toString();
	// String d= yiBaoUtility.getCashier(token,20158l);
	// Map<String,String> addda =
	// yiBaoUtility.getYiBaoOrder("hq2017090815262700180","1001201709080000000015990156");
	// String e="";
	...
	// HashMap srce =
	// huichaoUtility.createOrderZFB("zfaaaabcdef1","1",13989456178L,
	// PayOrderSource.RENEWAL_PAY);

...
public class StartCashierApi implements ApiHandle {
	...
	public ApiHandleResponse process(RequestDataVo requestDataVo,
		...
		if (orderType.equals(OrderType.AGENTBUY.getCode())
				|| orderType.equals(OrderType.SELFSUPPORT.getCode())
				|| orderType.equals(OrderType.TRADE.getCode())
				|| orderType.equals(OrderType.LEASE.getCode())) {
		...
		if (orderType.equals(OrderType.LEASE.getCode())) {
			if (orderInfo.getActualAmount().compareTo(BigDecimal.ZERO) == 0) {
				...
				if (useableAmount.compareTo(afOrderLeaseDo.getFreezeAmount()) >= 0) {
					...
					cashierVo.setAmount(afOrderLeaseDo
							.getFreezeAmount()
							.subtract(useableAmount)
							.add(afOrderLeaseDo.getMonthlyRent().add(afOrderLeaseDo.getRichieAmount())));
				}
			}
		}
		...
		if (StringUtil.equals(authDo.getFundStatus(), YesNoStatus.YES.getCode())
				&& StringUtil.equals(authDo.getJinpoStatus(),YesNoStatus.YES.getCode())
				&& StringUtil.equals(authDo.getCreditStatus(),YesNoStatus.YES.getCode())
				&& StringUtil.equals(authDo.getAlipayStatus(),YesNoStatus.YES.getCode())
	 ...
	 * @return
	 */
	private CashierTypeVo canCP(AfUserAccountDto userDto, AfUserAuthDo authDo,
			AfOrderDo orderInfo, AfCheckoutCounterDo checkoutCounter) {
		if (StringUtil.isEmpty(checkoutCounter.getCppayStatus())
				|| checkoutCounter.getCppayStatus().equals(YesNoStatus.NO.getCode())) {
	 ...
	 * @return
	 */
	private CashierTypeVo canWX(AfUserAccountDto userDto, AfUserAuthDo authDo,
			AfOrderDo orderInfo, AfCheckoutCounterDo checkoutCounter) {
		if (StringUtil.isEmpty(checkoutCounter.getWxpayStatus())
				|| checkoutCounter.getWxpayStatus().equals(YesNoStatus.NO.getCode())) {
			return new CashierTypeVo(YesNoStatus.NO.getCode(),CashierReasonType.CASHIER.getCode());
		}
		return new CashierTypeVo(YesNoStatus.YES.getCode());
	}
	 ...
	 * @return
	 */
	private CashierTypeVo canBankpay(AfUserAccountDto userDto,
			AfUserAuthDo authDo, AfOrderDo orderInfo,
			AfCheckoutCounterDo checkoutCounter) {
		if (StringUtil.isEmpty(checkoutCounter.getBankpayStatus())
				|| checkoutCounter.getBankpayStatus().equals(YesNoStatus.NO.getCode())) {
			return new CashierTypeVo(YesNoStatus.NO.getCode(),CashierReasonType.CASHIER.getCode());
		}
		return new CashierTypeVo(YesNoStatus.YES.getCode());
	}
	 ...
	 * @return
	 */
	private CashierTypeVo canAlipay(AfUserAccountDto userDto,
			AfUserAuthDo authDo, AfOrderDo orderInfo,
			AfCheckoutCounterDo checkoutCounter) {
		if (StringUtil.isEmpty(checkoutCounter.getAlipayStatus())
				|| checkoutCounter.getAlipayStatus().equals(YesNoStatus.NO.getCode())) {
			return new CashierTypeVo(YesNoStatus.NO.getCode(),CashierReasonType.CASHIER.getCode());
		}
		return new CashierTypeVo(YesNoStatus.YES.getCode());
	}
	 ...
	 * @return
	 */
	private CashierTypeVo canConsume(AfUserAccountDto userDto,
			AfUserAuthDo authDo, AfOrderDo orderInfo,
			AfCheckoutCounterDo checkoutCounter, AfInterimAuDo afInterimAuDo,
		...
		if (StringUtil.isEmpty(checkoutCounter.getInstallmentStatus())
				|| checkoutCounter.getInstallmentStatus().equals(YesNoStatus.NO.getCode())
				|| orderInfo.getOrderType().equals(OrderType.LEASE.getCode())) {
		...
		if (status.equals(YesNoStatus.YES.getCode())) {
			...
			BigDecimal minAmount = consumeMinResource == null ? BigDecimal.ZERO
					: new BigDecimal(consumeMinResource.getValue());
			...
			BigDecimal usabledMinAmount = usabledMinResource == null ? BigDecimal.ZERO
					: new BigDecimal(usabledMinResource.getValue());
			...
			return new CashierTypeVo(YesNoStatus.NO.getCode(),
					CashierReasonType.NEEDAUTH.getCode());
		}
	}
	 ...
	 * @return
	 */
	private CashierTypeVo canCredit(AfUserAccountDto userDto,
			AfUserAuthDo authDo, AfOrderDo orderInfo,
			AfCheckoutCounterDo checkoutCounter) {
		// 关闭信用支付
		// if (StringUtil.isEmpty(checkoutCounter.getCreditStatus()) ||
		// checkoutCounter.getCreditStatus().equals(YesNoStatus.NO.getCode())) {
		return new CashierTypeVo(YesNoStatus.NO.getCode(),
				CashierReasonType.CASHIER.getCode());
		// }
		// // 分期金额限制
		// String nper = getNperListApi.checkMoneyLimit(new JSONArray(),
		// orderInfo.getOrderType(), orderInfo.getActualAmount());
		// if ("0".equals(nper)) {
		// return new CashierTypeVo(YesNoStatus.NO.getCode(),
		// CashierReasonType.CASHIER.getCode());
		// }
		// AfUserBankcardDo card =
		// afUserBankcardService.getUserMainBankcardByUserId(orderInfo.getUserId());
		// String cardNo = card.getCardNumber();
		// String riskOrderNo = riskUtil.getOrderNo("crep",
		// cardNo.substring(cardNo.length() - 4, cardNo.length()));
		// String state = riskUtil.creditPayment(userDto.getUserId().toString(),
		// riskOrderNo);
		// if (state.equals(YesNoStatus.YES.getCode())) {
		// CashierTypeVo cashierTypeVo = new
		// CashierTypeVo(YesNoStatus.YES.getCode());
		// cashierTypeVo.setPayAmount(orderInfo.getActualAmount());
		// return cashierTypeVo;
		// } else {
		// CashierTypeVo cashierTypeVo = new
		// CashierTypeVo(YesNoStatus.NO.getCode(),
		// CashierReasonType.RISK_CREDIT_PAYMENT.getCode());
		// return cashierTypeVo;
		// }
	 ...
	 * @return
	 */
	private String getIsAuth(AfUserAccountDto userDto, AfUserAuthDo authDo,
			AfOrderDo orderInfo, FanbeiContext context) {
		String status = YesNoStatus.NO.getCode();
		if (context.getAppVersion() >= 406) {
			// 获取不同场景的强风控认证
			if (orderInfo.getOrderType().equals(OrderType.TRADE.getCode())) {
				// 商圈认证
				AfUserAuthStatusDo afUserAuthStatusDo = afUserAuthStatusService
						.selectAfUserAuthStatusByCondition(userDto.getUserId(),
				...
				AfUserAuthStatusDo afUserAuthStatusDo = afUserAuthStatusService
						.selectAfUserAuthStatusByCondition(userDto.getUserId(),
								UserAccountSceneType.ONLINE.getCode(),
								YesNoStatus.YES.getCode());
				if (afUserAuthStatusDo == null) {
					authDo.setRiskStatus(YesNoStatus.NO.getCode());
				} else {
		...
		if (YesNoStatus.YES.getCode().equals(authDo.getRiskStatus())) {
			if (StringUtil.equals(YesNoStatus.YES.getCode(),
					authDo.getZmStatus())// 芝麻信用已验证
					&& StringUtil.equals(YesNoStatus.YES.getCode(),
							authDo.getTeldirStatus())// 通讯录匹配状态
					&& StringUtil.equals(YesNoStatus.YES.getCode(),
							authDo.getMobileStatus())// 手机运营商
					&& StringUtil.equals(YesNoStatus.YES.getCode(),
							authDo.getRiskStatus())) { // 强风控状态
	 ...
	 *            用户账户信息
	 */
	private Map<String, Object> riskProcess(CashierTypeVo cashierTypeVo,
			AfOrderDo orderInfo, AfUserAccountDto userDto,
			BigDecimal usabledMinAmount, AfInterimAuDo afInterimAuDo) {
		// 风控逾期订单处理
		// RiskQueryOverdueOrderRespBo resp =
		// riskUtil.queryOverdueOrder(orderInfo.getUserId() + StringUtil.EMPTY);
		Long userId = orderInfo.getUserId();
		// String rejectCode = "";
		// region 测试借钱逾期白名单
		// List<AfResourceDo> afResourceList =
		// afResourceService.getConfigByTypes("overduecash_test_user");
		// if (afResourceList.size() > 0) {
		// if (afResourceList.get(0).getValue() != null &&
		// afResourceList.get(0).getValue().contains(String.valueOf(userDto.getUserName())))
		// {
		...
		// region 测试分期逾期白名单
		// List<AfResourceDo> afResourceList1 =
		// afResourceService.getConfigByTypes("overdue_test_user");
		// if (afResourceList1.size() > 0) {
		// if (afResourceList1.get(0).getValue() != null &&
		// afResourceList1.get(0).getValue().contains(String.valueOf(userDto.getUserName())))
		// {
		// rejectCode = RiskErrorCode.OVERDUE_BORROW.getCode();
		// }
		// }
		...
		// RiskErrorCode erorrCode =
		// RiskErrorCode.findRoleTypeByCode(rejectCode);
		...
		// AfBorrowDo borrowInfo =
		// afBorrowService.getOverdueBorrowInfoByUserId(userId);
		// if (borrowInfo != null) {
		// Long billId =
		// afBorrowBillService.getOverduedAndNotRepayBillId(borrowInfo.getRid());
		...
		if (borrowBillInfo != null) {
			...
			if (cashInfo != null && "Y".equals(cashInfo.getOverdueStatus())) {
				...
				BigDecimal allAmount = BigDecimalUtil.add(cashInfo.getAmount(),
						cashInfo.getSumOverdue(), cashInfo.getOverdueAmount(),
						cashInfo.getRateAmount(), cashInfo.getSumRate());
				...
				logger.error("cashier error: risk overdueBorrowCash not found in fanbei,risk userId:"
						+ userDto.getUserId());
				// logger.error("cashier error: risk overdueBorrow not found in fanbei,risk borrowBo:"
				// + JSONObject.toJSONString(borrowInfo));
				logger.error("cashier error: risk overdueBorrow not found in fanbei,risk borrowBillBo:"
						+ JSONObject.toJSONString(borrowBillInfo));
			}
		...
		if (userAccountInfo != null) {
			...
			if ("TRUE".equals(virtualMap.get(Constants.VIRTUAL_CHECK))) {
				...
				cashierTypeVo.setTotalVirtualAmount(virtualMap.get(Constants.VIRTUAL_TOTAL_AMOUNT) == null ? 
						new BigDecimal(virtualMap.get(Constants.VIRTUAL_AMOUNT).toString()) : new BigDecimal(virtualMap.get(Constants.VIRTUAL_TOTAL_AMOUNT).toString()));

			} else {
				cashierTypeVo.setIsVirtualGoods(YesNoStatus.NO.getCode());
				cashierTypeVo.setUseableAmount(leftAmount);
				cashierTypeVo.setPayAmount(leftAmount.compareTo(orderInfo.getActualAmount()) > 0 ? 
						orderInfo.getActualAmount() : leftAmount);
	 ...
	 * @return
	 */
	private BigDecimal getUseableAmount(AfOrderDo orderInfo,
			AfUserAccountDto userDto, AfInterimAuDo afInterimAuDo) {
		...
		if (orderInfo.getOrderType().equals(OrderType.TRADE.getCode())) {
			// 教育培训订单
			if (orderInfo.getSecType().equals(
					UserAccountSceneType.TRAIN.getCode())) {
				AfUserAccountSenceDo afUserAccountSenceDo = afUserAccountSenceService.getByUserIdAndType(
								UserAccountSceneType.TRAIN.getCode(),
								userDto.getUserId());
				if (afUserAccountSenceDo != null) {
					useableAmount = afUserAccountSenceDo.getAuAmount()
							.subtract(afUserAccountSenceDo.getUsedAmount())
			...
			// 线上分期订单
			/*
			 * //mqp add switch for different scene without TRADE String
			 * orderType = orderInfo.getOrderType(); String secOrderType =
			 * orderInfo.getSecType();
			 * 
			 * if (orderInfo.equals("SELFSUPPORT")) { secOrderType =
			 * "SELFSUPPORT"; }
			 * 
			 * AfCheckoutCounterDo checkoutCounterDo =
			 * afCheckoutCounterService.getByType(orderType, secOrderType); if
			 * (checkoutCounterDo == null) {
			 * logger.info("checkUsedAmount checkoutcounterdo is null"); throw
			 * new
			 * FanbeiException(FanbeiExceptionCode.TEMPORARY_AMOUNT_SWITH_EMPTY
			 * ); }
			 * 
			 * String isSwitch = checkoutCounterDo.getTemporaryAmountStatus();
			 * 
			 * if (StringUtil.isEmpty(isSwitch)) {
			 * logger.info("checkUsedAmount isSwitch is null"); throw new
			 * FanbeiException
			 * (FanbeiExceptionCode.TEMPORARY_AMOUNT_SWITH_EMPTY); }
			 * 
			 * //end mqp add switch for different scene
			 */

			AfUserAccountSenceDo afUserAccountSenceDo = afUserAccountSenceService
					.getByUserIdAndType(UserAccountSceneType.ONLINE.getCode(),
							userDto.getUserId());

			if (afUserAccountSenceDo != null) {
				// 额度判断
				if (afInterimAuDo.getGmtFailuretime().compareTo(
						DateUtil.getToday()) >= 0) {
					// 获取当前用户可用临时额度
					BigDecimal interim = afInterimAuDo.getInterimAmount()
							.subtract(afInterimAuDo.getInterimUsed());
					useableAmount = afUserAccountSenceDo.getAuAmount()
							.subtract(afUserAccountSenceDo.getUsedAmount())
							.subtract(afUserAccountSenceDo.getFreezeAmount())
							.add(interim);

...
public abstract class BaseController {
	...
	protected String processRequest(String reqData, HttpServletRequest request, boolean isForQQ) {
		...
		try {
			...
			//406强升需要数据拦截的借钱相关接口
			String apiUrl = "/legalborrowV2/confirmLegalRenewalPay,/legalborrow/confirmLegalRenewalPay,/borrowCash/applyBorrowCashV1,/borrowCash/confirmRenewalPay";
			...
			try {
				...
				if (afResourceDo != null && afResourceDo.getValue().toLowerCase().equals("true")) {
					if (contex.getAppVersion() < Integer.parseInt(afResourceDo.getValue1())
							&& requestDataVo.getId().endsWith("www")) {
						if (!(request.getRequestURI().toString().toLowerCase().contains("/system/appupgrade")
								|| request.getRequestURI().toLowerCase().contains("/system/checkversion"))) {
			...
			try {
				...
				if (biLogger.isInfoEnabled()) {
					...
					String clientType = "o";
					if (requestDataVo != null && requestDataVo.getId() != null
							&& requestDataVo.getId().startsWith("i_")) {
						clientType = "i";
					} else if (requestDataVo != null && requestDataVo.getId() != null
							&& requestDataVo.getId().startsWith("a_")) {
						clientType = "a";
					}
					biNewLog.info(StringUtil.appendStrs("	",
							DateUtil.formatDate(calStart.getTime(), DateUtil.DATE_TIME_SHORT), "	", clientType,
							"	", CommonUtil.getIpAddr(request), "	", userName, "	",
							(calEnd.getTimeInMillis() - calStart.getTimeInMillis()), "	", url, "	",
							exceptionresponse == null ? 9999
									: ((ApiHandleResponse) exceptionresponse).getResult().getCode(),
					...
					biLogger.info(StringUtil.appendStrs("	",
							DateUtil.formatDate(calStart.getTime(), DateUtil.DATE_TIME_SHORT), "	", clientType,
							"	rmtIP=", CommonUtil.getIpAddr(request), "	userName=", userName, "	",
							(calEnd.getTimeInMillis() - calStart.getTimeInMillis()), "	", url, "	result=",
							exceptionresponse == null ? 9999
									: ((ApiHandleResponse) exceptionresponse).getResult().getCode(),
							"	", DateUtil.formatDate(new Date(), DateUtil.MONTH_SHOT_PATTERN), "	", "", "	", "",
							"	", "", "	", "", "	", "", "	reqD=", req, "	resD=",
	 ...
	 * @return
	 */
	public abstract BaseResponse doProcess(RequestDataVo requestDataVo, FanbeiContext context,
	 ...
	 */
	private void checkH5Sign(HttpServletRequest request, FanbeiH5Context h5Context, RequestDataVo requestDataVo,
			boolean needToken) {
	...
	private FanbeiContext doSystemCheck(RequestDataVo requestDataVo) {
		...
		if ("/system/appUpgrade".equals(requestDataVo.getMethod())
				&& "379".equals(systemMap.get(Constants.REQ_SYS_NODE_VERSION))
				&& (requestDataVo.getId() != null && requestDataVo.getId().startsWith("i_"))) {

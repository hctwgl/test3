...
public class ApplyLegalBorrowCashApi extends GetBorrowCashBase implements ApiHandle {
	...
	private void delegatePay(String consumerNo, String orderNo, String result,
		...
		if (whiteIdsList.contains(afUserDo.getUserName()) || StringUtils.equals("10", result)) {
			...
			logger.info("title="+title+"&bankCardNo="+bankinfo.getBankCode().substring(bankinfo.getBankCode().length()-4, bankinfo.getBankCode().length()));
			msgContent.replace("&bankCardNo", bankinfo.getBankCode().substring(bankinfo.getBankCode().length()-4, bankinfo.getBankCode().length()));

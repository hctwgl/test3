...
public class PayOrderV1Api implements ApiHandle {
	...
	public ApiHandleResponse process(RequestDataVo requestDataVo, FanbeiContext context, HttpServletRequest request) {
		...
		String type = ObjectUtils.toString(requestDataVo.getParams().get("type"), OrderType.BOLUOME.getCode())
				.toString();
		...
		String isCombinationPay = ObjectUtils.toString(requestDataVo.getParams().get("isCombinationPay"), "")
				.toString();
		...
		if (payId >= 0) {
			...
			if (!StringUtils.equals(inputOldPwd, userAccountInfo.getPassword())) {
				...
				if (OrderType.getNeedRecordPayFailCodes().contains(orderInfo.getOrderType())) {
					...
					// 支付失败
					// boluomeUtil.pushPayStatus(orderInfo.getRid(),
					// orderInfo.getOrderNo(), orderInfo.getThirdOrderNo(),
					// PushStatus.PAY_FAIL, orderInfo.getUserId(),
		...
		if (wxPayType.equals(PayType.WECHAT.getCode())) {
			...
			if (WxpayConfig.RESULT_CODE_SUCCESS.equals(resultCode) && (WxpayConfig.TRADE_STATE_SUCCESS.equals(tradeCode)
		...
		try {
			...
			if (StringUtils.equals(type, OrderType.AGENTBUY.getCode())
					|| StringUtils.equals(type, OrderType.SELFSUPPORT.getCode())
					|| StringUtils.equals(type, OrderType.TRADE.getCode())) {
				saleAmount = orderInfo.getActualAmount();
			}
			if (payId == 0 && (StringUtils.equals(orderInfo.getOrderType(), OrderType.SELFSUPPORT.getCode())
					|| StringUtils.equals(orderInfo.getOrderType(), OrderType.TRADE.getCode()) || nper == null)) {
			...
			Map<String, Object> result = afOrderService.payBrandOrder(payId, payType, orderInfo.getRid(),
					orderInfo.getUserId(), orderInfo.getOrderNo(), orderInfo.getThirdOrderNo(),
					orderInfo.getGoodsName(), saleAmount, nper, appName, ipAddress);
				...
				if (Boolean.parseBoolean(success.toString())) {
					...
					if (StringUtils.equals(type, OrderType.BOLUOME.getCode())) {
						if (payId.intValue() == 0) {
							...
							boluomeUtil.pushPayStatus(orderInfo.getRid(), orderInfo.getOrderNo(),
									orderInfo.getThirdOrderNo(), PushStatus.PAY_DEALING, orderInfo.getUserId(),
									orderInfo.getActualAmount());
						}
					}

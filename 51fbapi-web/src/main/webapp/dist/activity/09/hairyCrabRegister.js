...
var timerInterval, timerS = 60, vm = new Vue({
    el: "#hairyCrabRegister", data: {alertShow: ""}, methods: {
        getImgCode: function () {
            var e = $("#tel").val();
            var mobile=e;
            !isNaN(e) && /^1(3|4|5|7|8)\d{9}$/i.test(e) ? $.ajax({
                ...
                success: function (e) {
                    ...
                    ;
                    $(".registerMask").removeClass("hide"), $(".imgVftCodeWrap").removeClass("hide"), $("#imgVftCode").val(""), $("#imgVftCodeWrapImg").attr("src", "data:image/png;base64," + e.data), $("#imgVftCodeClose").click(function () {
            ...
            var e = this, t = $("#password").val().length, a = String(CryptoJS.MD5($("#password").val())),
                r = /^((?=.*?\d)(?=.*?[A-Za-z])|(?=.*?\d)(?=.*?[.!@#$%])|(?=.*?[A-Za-z])(?=.*?[.]))[\dA-Za-z.!@#$%]+$/.test($("#password").val()),
                s = $("#tel").val(), o = $("#VerifiCode").val();
            $("#register_codeBtn").attr("isState");
            /^1(3|4|5|7|8)\d{9}$/i.test(s) && "" != s ? "" != o ? r && 6 <= t && t <= 18 ? $("#inputCheck").is(":checked") ? 1 == $("#codeBtn").attr("isState") ? $.ajax({
                ...
                success: function (t) {
                    console.log(t), t.success ? ($("#register_submitBtn").attr("disabled", !0), e.alertShow = "Y", $(".goNow").click(function () {

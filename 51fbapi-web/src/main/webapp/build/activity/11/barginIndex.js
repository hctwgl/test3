...
let vm = new Vue({
    ...
    methods: {
        ...
        share: function(id, type, status) { // 点击发起砍价
            goodsId = id;
            goodsType = type;
            shareInfo.link = urlHost + '/fanbei-web/activity/barginProduct?goodsId=' + goodsId + '&productType=share' + goodsType + '&userName=' + userName + '&spread=' + spread;
            let self = this;
            if (status == 1) {
                self.shareSure();
            } else {
                    ...
                    success: function(data) {
                        if (!data.success) { // 返回不可砍價
                            if (!data.hasOwnProperty("data")) {
                                ...
                                if (self.isWX) {
                                    location.href = "./barginLogin?&spread=" + spread;
                                } else {

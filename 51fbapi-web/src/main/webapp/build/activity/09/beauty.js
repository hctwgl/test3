...
let vm = new Vue({
    ...
    methods: {
        ...
        logCoupon() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    self.$nextTick(function () {
                        for(let i=0;i<self.couponCont.couponInfoList.length;i++){
                            console.log(self.couponCont.couponInfoList[i].drawStatus);
                            if(self.couponCont.couponInfoList[i].drawStatus=='Y'){
                                $('.coupon').eq(i).addClass('gray');
                                $('.coupon').eq(i).find('.couponStatus').html('已领取');
                            }
                        }
                    })
                },
        ...
        couponClick:function(index) {
            let self = this;
            let couponId = self.couponCont.couponInfoList[index].couponId;
            $.ajax({
                ...
                success: function (returnData) {
                    if (returnData.success) {
                        requestMsg("优惠劵领取成功");
                        $('.coupon').eq(index).addClass('gray');
                        $('.coupon').eq(index).find('.couponStatus').html('已领取');
                    } else {

...
function alaShareData(){
    ...
    let dataObj = {
        ...
        'shareAppContent': '抓娃娃次数无上限100%中奖，集齐5娃，平分1亿大奖，最高888红包雨在等你，有且只在爱上街！',  // 分享的内容
        ...
        success:function (data) {
            ...
            if(data.success){
                ...
                if(data.data.item1Count>0){
                    $('#toys').find('img').eq(0).attr('src','https://f.51fanbei.com/h5/app/activity/06/ni_boll1.png');
                    $('#toys').find('span').eq(0).html(data.data.item1Count)
                }
                if(data.data.item2Count>0){
                    $('#toys').find('img').eq(1).attr('src','https://f.51fanbei.com/h5/app/activity/06/ni_boll2.png');
                    $('#toys').find('span').eq(1).html(data.data.item2Count)
                }
                if(data.data.item3Count>0){
                    $('#toys').find('img').eq(2).attr('src','https://f.51fanbei.com/h5/app/activity/06/ni_boll3.png');
                    $('#toys').find('span').eq(2).html(data.data.item3Count)
                }
                if(data.data.item4Count>0){
                    $('#toys').find('img').eq(3).attr('src','https://f.51fanbei.com/h5/app/activity/06/ni_boll4.png');
                    $('#toys').find('span').eq(3).html(data.data.item4Count)
                }
                if(data.data.item5Count>0){
                    $('#toys').find('img').eq(4).attr('src','https://f.51fanbei.com/h5/app/activity/06/ni_boll5.png');
...
class game{
    ...
    reset(){
        ...
        $('#claw').css('backgroundImage','url(https://f.51fanbei.com/h5/app/activity/06/ni_claw1.png)');  //还原钩子
    ...
    doll(){
        ...
        for(let i=0;i<dollNum.length;i++){
            con+=`<span data-prop="${dollNum[i]}" class="doll"><div class="doll-main" style="background-image: url('https://f.51fanbei.com/h5/app/activity/06/ni_boll${dollNum[i]}.png')"></div></span>`
        }
        $('#scroll').html(con)
    }
    alertMsg(state,item){
            ...
            $('.doll').each(function () {
                ...
                if(dollLeft>(clawLeft-10) && dollLeft<(clawLeft+35)){          //判断钩子与娃娃是否重合，减的越大越偏右
                    ...
                    if(Math.floor(Math.random()*100+1)<clientRate){                 //随机能否抓到娃娃
                        $('#claw').css('backgroundImage','url(https://f.51fanbei.com/h5/app/activity/06/ni_claw2.png)'); //钩子变为收缩样式
